...
PCERT_CONTEXT = POINTER(CertContext)
...
PCERT_ENHKEY_USAGE = POINTER(CertEnhkeyUsage)


class CertUsageMatch(Structure):
    _fields_ = (
        ("dwType", DWORD),
...
if TYPE_CHECKING:
    ...
    PCERT_CHAIN_PARA: type[_Pointer[CertChainPara]] = POINTER(CertChainPara)  # type: ignore[misc]
else:
    PCERT_CHAIN_PARA = POINTER(CertChainPara)


class CertTrustStatus(Structure):
    _fields_ = (
        ("dwErrorStatus", DWORD),
        ("dwInfoStatus", DWORD),
    )
...
class CertChainElement(Structure):
    _fields_ = (
        ("cbSize", DWORD),
...
PCERT_CHAIN_ELEMENT = POINTER(CertChainElement)


class CertSimpleChain(Structure):
    _fields_ = (
        ("cbSize", DWORD),
        ("TrustStatus", CertTrustStatus),
        ("cElement", DWORD),
...
PCERT_SIMPLE_CHAIN = POINTER(CertSimpleChain)


class CertChainContext(Structure):
    _fields_ = (
        ("cbSize", DWORD),
        ("TrustStatus", CertTrustStatus),
        ("cChain", DWORD),
...
PCERT_CHAIN_CONTEXT = POINTER(CertChainContext)
PCCERT_CHAIN_CONTEXT = POINTER(PCERT_CHAIN_CONTEXT)


class SslExtraCertChainPolicyPara(Structure):
    _fields_ = (
        ("cbSize", DWORD),
        ("dwAuthType", DWORD),
        ("fdwChecks", DWORD),
...
class CertChainPolicyPara(Structure):
    _fields_ = (
        ("cbSize", DWORD),
        ("dwFlags", DWORD),
...
PCERT_CHAIN_POLICY_PARA = POINTER(CertChainPolicyPara)


class CertChainPolicyStatus(Structure):
    _fields_ = (
        ("cbSize", DWORD),
        ("dwError", DWORD),
        ("lChainIndex", LONG),
...
PCERT_CHAIN_POLICY_STATUS = POINTER(CertChainPolicyStatus)


class CertChainEngineConfig(Structure):
    _fields_ = (
        ("cbSize", DWORD),
